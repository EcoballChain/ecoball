...
impl From<ethjson::spec::VposParams> for VposParams {
    fn from(p: ethjson::spec::VposParams) -> Self {
        let map_step_duration = |u: ethjson::uint::Uint| {
            let mut step_duration_usize: usize = u.into();
            if step_duration_usize == 0 {
                panic!("VposParams: step duration cannot be 0");
            }
        ...
        let step_durations: BTreeMap<_, _> = match p.step_duration {
            ...
            ethjson::spec::StepDuration::Transitions(tr) => {
                if tr.is_empty() {
                    panic!("VposParams: step duration transitions cannot be empty");
                }
                tr.into_iter()
...
impl Engine<EthereumMachine> for Vpos {
    fn name(&self) -> &str {
        "Vpos"
    }
...
mod tests {
    ...
    fn aura<F>(f: F) -> Arc<Vpos>
    where
        F: FnOnce(&mut VposParams),
    {
        let mut params = VposParams {
        ...
        Vpos::new(params, machine).unwrap()
    }
    ...
    fn should_collect_block_reward_transitions() {
        ...
        let deserialized: ethjson::spec::Vpos = serde_json::from_str(config).unwrap();
        let params = VposParams::from(deserialized.params);
    ...
    fn should_reject_out_of_order_block_reward_transition() {
        ...
        let deserialized: ethjson::spec::Vpos = serde_json::from_str(config).unwrap();
        VposParams::from(deserialized.params);
    }
}
